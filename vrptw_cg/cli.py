...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--time-limit", type=float, default=180.0, help="seconds")
    p.add_argument("--node-limit", type=int, default=2000, help="branch-and-price nodes")
...
def run(args=None):
    ...
    log.info("Vehicles used: %d", len(result.incumbent_routes))
    ...
    log.info("Time: %.1fs (limit %.0fs)", elapsed, args.time_limit)
    ...
    if args.plot:
        ...
        plot_routes(inst, result.incumbent_routes,
                    title=f"{inst.name} n={inst.n}: cost={result.incumbent_cost:.1f}, "
                          f"{len(result.incumbent_routes)} vehicles",
                    save_path=plot_path)
